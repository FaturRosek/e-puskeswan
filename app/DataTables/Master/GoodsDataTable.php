...
class GoodsDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addIndexColumn()
            ->addColumn('action', function (Goods $row) {
                return view('pages.admin.master.Goods.action', ['barang' => $row]);
            })
            ->rawColumns(['action'])
            ->setRowId('id');
    }
    ...
    public function getColumns(): array
    {
        return [
            Column::computed('DT_RowIndex')
                ->title('No.')
                ->width(20),
            Column::make('goods_name')->title('Nama Barang'),
            Column::make('goods_type')->title('Jenis'),
